...
- Los álbumes (varias fotos/videos juntos) se esperan a que lleguen completos
  antes de reenviarlos (el temporizador se reinicia con cada foto/video nuevo).
...
async def procesar_album(context: ContextTypes.DEFAULT_TYPE):
    ...
    if contador_multimedia:
        registrar_multimedia(excluir, contador_multimedia)

    encolar_para_todos("album", media_list, excluir, logical_id, entregas_respuesta)
...
async def manejar_mensaje(update, context):
    ...
    if msg.media_group_id:
        ...
        album_buffer[gid]["mensajes"].append(msg)
        registrar_entrega(album_buffer[gid]["logical_id"], chat_id, msg.message_id)

        # Reprogramamos el disparo: solo procesa cuando dejan de llegar fotos/videos
        # nuevos de este mismo álbum durante 1.5s (así no importa si el álbum es grande
        # y tarda más en llegar completo).
        for job in context.job_queue.get_jobs_by_name(f"album_{gid}"):
            job.schedule_removal()
        context.job_queue.run_once(procesar_album, when=1.5, data={"gid": gid}, name=f"album_{gid}")
        return
    ...
    if msg.photo:
        registrar_multimedia(user_id, 1)
        encolar_para_todos("photo", {"file_id": msg.photo[-1].file_id, "caption": formatear_caption(nombre, texto)},
                            user_id, logical_id, entregas_respuesta)
    elif msg.video:
        registrar_multimedia(user_id, 1)
        encolar_para_todos("video", {"file_id": msg.video.file_id, "caption": formatear_caption(nombre, texto)},
                            user_id, logical_id, entregas_respuesta)
    elif msg.document:
